...
function renderDashboard() {
  ...
  document.getElementById('group-filter-container').innerHTML =
    `<div class="filter-chip filter-all ${fG===null?'active':''}" onclick="setGroupFilter(null)">全体表示</div>` +
    groups.map(g => `<div class="filter-chip ${fG===g?'active':''}" onclick="setGroupFilter('${g}')">${g}</div>`).join('');
  const sortedTopics = [...state.topics].sort((a,b) => a.localeCompare(b, 'ja'));
  document.getElementById('topic-filter-container').innerHTML =
    `<div class="filter-chip filter-all ${fT===null?'active':''}" onclick="setTopicFilter(null)">全体表示</div>` +
    sortedTopics.map(t => `<div class="filter-chip ${fT===t?'active':''}" onclick="setTopicFilter('${t}')">${t}</div>`).join('');
...
function renderStudentTable() {
  ...
  // テキストフィルター入力を監視
  const filterInput = document.getElementById('table-filter-text');
  if (filterInput && !filterInput._listenerAdded) {
    filterInput._listenerAdded = true;
    filterInput.addEventListener('input', () => { filterText = filterInput.value; renderStudentTable(); });
  }
}
...
window.loadRoster = loadRosterIntoGrid;
window.loadRosterIntoGrid = loadRosterIntoGrid;
window.saveSession = addTopic;
window.deleteSession = deleteTopic;
...
window.setChartGroup = setChartGroup;
window.highlightDetail = highlightDetail;
